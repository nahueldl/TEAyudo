...
const categoriaDAO = {
	...
	getAll: async function (id_usuario, id_paciente = null){
		...
		if(id_paciente === null)
			...
			return await genericDAO.runQuery("select * from (select 0 as id_categoria, null as id_usuario_rol, 'Favoritos' as nombre, null as fecha_hora_alta, 0 as cat where exists(select * from Pictograma_Paciente where id_paciente = @idPaciente and favorito is not null and favorito != 0) union select ca.id_categoria, ca.id_usuario_rol, ca.nombre, ca.fecha_hora_alta, id_categoria as cat from Categoria ca left join Usuario_Rol ur on ca.id_usuario_rol = ur.id_usuario_rol left join Rol ro on ro.id_rol = ur.id_rol left join Rol_Paciente rp on rp.id_usuario_rol = ur.id_usuario_rol where (ca.id_usuario_rol is null or ur.id_usuario = @id or rp.id_paciente = @idPaciente) and ca.activo = 1 group by ca.id_categoria, ca.id_usuario_rol, ca.nombre, ca.fecha_hora_alta ) as t order by CASE WHEN cat = 0 THEN 1 ELSE 0 END desc, CASE WHEN id_usuario_rol IS NOT NULL THEN 1 ELSE 0 END desc, (select count(*) from Traduccion_Pictograma tp inner join Pictograma p on p.id_pictograma = tp.id_pictograma inner join Categoria_Pictograma cp on cp.id_pictograma = p.id_pictograma inner join Traduccion t on t.id_traduccion = tp.id_traduccion where cp.id_categoria = cat and t.id_paciente = @idPaciente) desc, (select count(*) from Traduccion_Pictograma tp inner join Pictograma p on p.id_pictograma = tp.id_pictograma inner join Categoria_Pictograma cp on cp.id_pictograma = p.id_pictograma where cp.id_categoria = cat) desc, nombre asc", params);
	},
